...
function AdminProposeAgenda() {
  ...
  const fetchProposeAgenda = () => {
    axios
      .get(`http://localhost:8000/api/proposeagenda`)
      .then((response) => {
    ...
    try {
      ...
      if (editId) {
        response = await axios.post(
          `http://localhost:8000/api/proposeagenda/${editId}`,
          formDataToSend,
        ...
        response = await axios.post(
          "http://localhost:8000/api/proposeagenda",
          formDataToSend,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
        Swal.fire({
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:8000/api/proposeagenda/${id}`)
          .then(() => {
            fetchProposeAgenda();
